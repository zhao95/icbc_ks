...
/**
 * trigger history for quartz
 * @author liwei
 */

public class TriggerListenerPlugin implements SchedulerPlugin, TriggerListener {
	...
	private String name;
	/** current service */
//	private static final String CURRENT_SERV = "SY_COMM_SCHED_TRIGGER";

	/** log */
	private static Log log = LogFactory.getLog(TriggerListenerPlugin.class);
...
//			param.set("START_TIME", DateUtils.getDatetime(trigger.getStartTime()));
//		}
//		if (null != trigger.getEndTime()) {
//			param.set("END_TIME", DateUtils.getDatetime(trigger.getEndTime()));
//		}
//		if (trigger instanceof SimpleTrigger) {
//			SimpleTrigger sTrig = (SimpleTrigger) trigger;
//			param.set("SIMPLE_TRIGGER_REPEAT_COUNT", sTrig.getRepeatCount());
//		}
//		ServDao.update(CURRENT_SERV, param);
	}
...
//		Bean param = new Bean();
//		param.setId(trigger.getKey().getName());
//		ServDao.deletes(CURRENT_SERV, param);
	}
