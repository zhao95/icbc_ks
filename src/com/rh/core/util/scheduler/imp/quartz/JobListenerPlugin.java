 ...
 * @author liwei
 */
public class JobListenerPlugin implements SchedulerPlugin, JobListener {

    /** plugin name */
    private String name;
    /** current service */
    private static final String CURRENT_SERV = "SY_COMM_SCHED_HIS";

    /** log */
    private static Log log = LogFactory.getLog(JobListenerPlugin.class);

    /** can new instance */
    public JobListenerPlugin() {
    }
    ...
    public void jobWasExecuted(JobExecutionContext context, JobExecutionException jobException) {
        ...
        try {
            log.debug("jobWasExecuted, update trigger state...");
            getQuartzScheduler().updateState(trigger);
        ...
        try {
            ...
            getQuartzScheduler().updateState(context.getJobDetail().getKey());
    ...
    private void saveHistory(JobExecutionContext context, boolean successed, String desc) {
        ...
        paramBean.set("SCHED_NAME", SchedulerMgr.getGlobalScheduler().getSchedulerName());
    ...
    private QuartzScheduler getQuartzScheduler() {
        IScheduler obj = SchedulerMgr.getGlobalScheduler();
