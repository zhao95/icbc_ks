...
public class DictMgr {
        ...
        if (dict == null) {
            ...
            if (dict != null) {
                if (dict.isNotEmpty("DICT_CLASS")) {
                    try {
                        DictListener lsnr = 
                                (DictListener) Lang.createObject(DictListener.class, dict.getStr("DICT_CLASS"));
    ...
    private static Bean getItemCmpyBean(String cmpy, Bean dict) {
        ...
        if ((dict.getInt("DICT_LOAD_TYPE") == Constant.NO_INT) 
                || !Context.isEmpytyThread(THREAD_DICT_EXT_WHERE)) { //=======临时使用的字典
            bCache = false;
        } else {
            bCache = true;
        }
    ...
    public static Bean loadItem(Bean dict, String cmpyCode) {
        ...
        if (dict.getInt("DICT_IS_INNER") == Constant.YES_INT) { //内部字典(必须为公共字典），执行内部初始化
            ...
            for (Bean item : itemList) {
                ...
                nItem.set("ID", item.getStr("ITEM_CODE")).set("NAME", item.getStr("ITEM_NAME"))
                    .set("PID", item.getStr("ITEM_PCODE"));
    ...
    private static String getDictItemListWhere(Bean dict, String cmpyCode) {
        ...
        if (cmpyCode != null && cmpyCode.length() > 0 && dict.isNotEmpty(COL_DICT_F_CMPY)) {
            sqlBuilder.append("").append(dict.getStr(COL_DICT_F_CMPY)).append("='")
                .append(cmpyCode).append("' ");
        } else {
...
//        }
     ...
     * @param dict 字典定义
     * @param itemList 字典项列表
     * @param autoFlag true自动根据有无子设置 叶子，false为组合树强制设定所有点为枝节点
     * @return 树形的字典数据
     */
    @SuppressWarnings("unchecked")
    ...
    public static List<Bean> buildItemLevel(Bean item) {
        ...
        if (item.contains(CHILD_NODE)) {
            ...
            for (Bean sub : subList) {
                ...
                sub.set("CODE_PATH", item.getStr("CODE_PATH") + sub.getStr("ID")
    ...
    public static synchronized void initDictCacheByDB() {
        ...
        String itemSql = "select * from " + TABLE_DICT_ITEM 
            + " where DICT_ID in (select DICT_ID from " +  TABLE_DICT + " where " 
            + "S_FLAG=1 and DICT_IS_INNER=1) order by DICT_ID,S_CMPY,ITEM_ORDER";
        Context.getExecutor().query(sql,
            new QueryCallback() {
            public void call(List<Bean> cols, Bean data) {
                setToCache(data.getStr("DICT_ID"), data);
            }
        });
        Context.getExecutor().query(itemSql,
            new QueryCallback() {
            public void call(List<Bean> cols, Bean data) {
     ...
     * @param pid 父编码
     * @param disLayer 一共显示几级
     * @param showPid 如果设定了父编码，则true:输出父编码作为根，false（缺省）:不输出父编码节点
     * @return 数据字典树
     */
    ...
    public static List<Bean> recurSubList(Bean dict, List<Bean> treeList, int layer) {
        ...
        String subExp = (dict.isNotEmpty("DICT_CHILD_ID")) ? getDict(dict.getStr("DICT_CHILD_ID"))
    ...
    public static Bean getItem(Bean dict, String code) {
        ...
        if (dict != null) {
            ...
            if (nodeMap != null) {
                ...
                if (value != null) {
                    item = (Bean) value;
                } else if (dict.getInt("DICT_IS_INNER") != Constant.YES_INT 
                        && dict.isNotEmpty("DICT_F_FLAG")) { //外部字典且启用标志字段
...
//                    }
    ...
    public static String getDictLikeSql(String dictId, String value) {
        ...
        if (dict.getInt("DICT_IS_INNER") == Constant.YES_INT) { //内部字典，执行内部过滤脚本
            ...
            if (dict != null) {
                ...
                sb.append("select ").append(dict.getStr(COL_DICT_F_ID)).append(" from ")
                    .append(dbUserName).append(dict.getStr(COL_TABLE_ID))
                    .append(getDictItemListWhere(dict, Context.getCmpy()))
                    .append(" and ").append(dict.getStr(COL_DICT_F_NAME))
    ...
    public static String getDictSubSql(Bean dict, String value) {
        ...
        if (dict.getInt("DICT_IS_INNER") == Constant.YES_INT) { //内部字典，执行内部过滤脚本
            ...
            sb.append("select ").append(dict.getStr(COL_DICT_F_ID)).append(" from ")
                .append(dbUserName).append(dict.getStr(COL_TABLE_ID))
                .append(getDictItemListWhere(dict, Context.getCmpy()))
                .append(" and ").append(dict.getStr("DICT_F_PATH"))
                .append(" like '").append(codePath).append("%'");
        }
        return sb.toString();
    }
     ...
     * @param dict 数据字典
     * @param itemCode 字典值
     * @param addedItem 已经增加的数据
     * @return 递归获取完整的路径信息
     */
    public static String getCodePath(Bean dict, String itemCode
            , HashSet<String> addedItem) {
    ...
    public static void addRebuild(String dictId, String cmpy) {
        Map<String, String> cmpyMap = (Map<String, String>) 
                CacheMgr.getInstance().get(dictId, CACHE_TO_REBUILD);
        if (cmpyMap == null) {
