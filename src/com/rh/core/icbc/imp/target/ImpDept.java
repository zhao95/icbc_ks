...
public class ImpDept {
	private static Log log = LogFactory.getLog(ImpDept.class);
...
//		Transaction.end();
	 ...
	 * 清除SY_ORG_DEPT表中ICBC的数据
	 */
	public void cleanDeptData() {
		 ParamBean whereBean = new ParamBean();
		 whereBean.setWhere(" AND CMPY_CODE = '" + ImpDept.CMPY_CODE + "' AND DEPT_CODE != '" + ImpDept.CMPY_CODE + "0001'");
...
//	}
	...
	public void resetAdminDept() {
		try {
			...
			log.error("重置admin的所在部门 ----"+e.getMessage());
		}
	}
     ...
     * @param pBean 父部门信息
     */
    private void buildSubList(List<Bean> list, Bean pBean, List<Bean> sortList) {
...
//                    .set("DEPT_TYPE", Constant.DEPT_TYPE_ORG)
     ...
     * 增量导入部门数据
     */
    public void addDeptDatas(String smtime) {
    	int count = 0;
    	...
    	for (int i=0; i<incrementDeptDatas.size(); i++) {
    		count += addIncrementDeptBean(incrementDeptDatas.get(i), errorCodes, finishCodes, 0);
    	}
     ...
     */
    public void resetDeptData(String struId) {
    	int count = 0;
     ...
     * @return
     */
    private List<Bean> getIncrementDeptDatas(String where) {
    	ParamBean whereBean = new ParamBean();
     ...
     * @return - 总处理量
     */
    private int addIncrementDeptBean(Bean incrementDeptBean, List<String> errorCodes, List<String> finishCodes, int count) {
		...
		if (oldDeptBean == null) { // 不存在，新增状态
			if (newDeptBean.getInt("S_FLAG") == 1) { // 有效数据，处理
				...
				if (newPDeptBean == null) { // 如果新增部门的父部门在本系统中不存在
					...
					if (incrementPDeptBean == null) { // 在中间表中也不存在
						...
						count = addIncrementDeptBean(incrementPDeptBean, errorCodes, finishCodes, count);
						finishCodes.add(newDeptBean.getStr("DEPT_PCODE")); // 将已经处理完成的父部门ID放入集合中，下次遍历到此条数据就跳过
						return addIncrementDeptBean(incrementDeptBean, errorCodes, finishCodes, count);
					}
			...
			if (newDeptBean.getStr("DEPT_PCODE").equals(oldDeptBean.getStr("DEPT_PCODE"))) { // 如果父部门没有改变
				...
				Bean newPDeptBean = ServDao.find(ServMgr.SY_ORG_DEPT, newDeptBean.getStr("DEPT_CODE"));
				if (newPDeptBean == null) { // 如果新增部门的父部门在本系统中不存在
					Bean incrementPDeptBean = ServDao.find(BOM_CMPSTRUINFO, newDeptBean.getStr("DEPT_PCODE"));
					if (incrementPDeptBean == null) { // 在中间表中也不存在
						errorCodes.add(newDeptBean.getStr("DEPT_CODE")); // 将未能同步的数据ID放入错误集合中
     ...
     * @return - 是否存在
     */
    private boolean checkCodeInError(List<String> errorCodes, Bean deptBean) {
	...
	private ParamBean setException(ParamBean odept) {
		...
		List<Bean> typeExceptions = ServDao.finds(ServMgr.SY_ORG_TYPE_EXCEPTION, " and S_FLAG = '1' and DEPT_CODE = '" + deptCode + "'");
		if (typeExceptions.size() > 0) {
			odept.set("DEPT_TYPE", typeExceptions.get(0).getInt("DEPT_TYPE"));
		}
		
		List<Bean> pidExceptions = ServDao.finds(ServMgr.SY_ORG_PID_EXCEPTION, " and S_FLAG = '1' and DEPT_CODE = '" + deptCode + "'");
		if (pidExceptions.size() > 0) {
			odept.set("DEPT_PCODE", pidExceptions.get(0).getStr("PDEPT_CODE"));
		}
		return odept;
	}
}
