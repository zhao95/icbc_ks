...
public class HrmNotesTask extends AbstractTableTask {
	...
	public HrmNotesTask(String smtime, boolean flag) {
		super(smtime, flag);
	}

	@Override
	protected String getSourceServId() {
		return HRM_ZDSTAFFNOTES;
	}

	@Override
	public String getTargetServId() {
		return SY_HRM_ZDSTAFFNOTES;
	}

	@Override
	protected String[] getPKCodeItems() {
		String[] pkItem = {"SSIC_ID","NOTES"};
		return pkItem;
	}
	...
	public boolean impIncData() {
		...
		try {
			...
			for (int i = 1; i <= total / SAVE_COUNT + 1; i++) {
				...
				ServDao.findsCall("HRM_ZDSTAFFNOTES", param, new RowHandler() {
					...
					public void handle(List<Bean> columns, Bean data) {
						...
						if (!deleteMap.containsKey(ssicId)) {
							ServDao.deletes("SY_HRM_ZDSTAFFNOTES",
				...
				// 批量添加入数据库
				ServDao.creates("SY_HRM_ZDSTAFFNOTES", addList);
			}
			Transaction.commit();
		} catch (Exception e) {
			Transaction.rollback();
			log.error("import SY_HRM_ZDSTAFFNOTES increment error ! " + e.getMessage());
			OrgLogMgr.orgLogSave("", "CC_DATA_IMPORT", "import SY_HRM_ZDSTAFFNOTES increment error ! " + e.getMessage(),
