...
public class QjlbServ extends CommonServ {
    ...
    public OutBean addData(ParamBean paramBean) {
        ...
        qjbean.set("USER_CODE", userWorkNum);//人力资源编码  todo  是不是要改个名称
...
//        String qjId = qjbd.getStr("QJ_ID");
        //起草人
        UserBean userBean = UserMgr.getUserByWorkNum(userWorkNum);
...
//        String usercode = paramBean.getStr("usercode");
    ...
    private void doFlowTask(OutBean outBean, UserBean userBean, Bean qjbean, int level) {
        ...
        String examerWorkNum = (String) qjbean.get("USER_CODE");//考生工号
        String shUserWorkNum = userBean.getWorkNum();

        ParamBean flowParamBean = new ParamBean();
        flowParamBean.set("examerWorekNum", examerWorkNum);
        flowParamBean.set("shrWorekNum", shUserWorkNum);//起草节点 shrWorekNum 传 examerWorekNum
...
//        new FlowServ().backFlow(flowParamBean);
        ...
        for (Bean bean : shList) {
            ...
            String shrWorknum = (String) bean.get("SHR_WORKNUM");
            shrNames.append(" ").append(shrName);
            //推送人
            UserBean shrUserBean = UserMgr.getUserByWorkNum(shrWorknum);
...
//            todoBean.set("SERV_ID", "0");//???
            ...
            todoBean.set("SEND_USER", userBean.getWorkNum());//发送人编码
            ...
            todoBean.set("OWNER_CODE", shrWorknum);
...
//            String bmLb = (String) bmBean.get("BM_LB");
    ...
    public OutBean getUserCanLeaveList(ParamBean paramBean) throws ParseException {
        ...
        List<Bean> tsBmshPassList = ServDao.finds("TS_BMSH_PASS", "and BM_CODE='" + userWorkNum + "'");//userWorkNum
