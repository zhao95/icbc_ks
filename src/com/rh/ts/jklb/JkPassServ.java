...
public class JkPassServ extends CommonServ {
    ...
    public OutBean expAll(ParamBean paramBean) {
        ...
        colMap.put("JKKS_NAME", "考试名称");
        colMap.put("JK_JKCITY", "借考的城市");
        colMap.put("JK_TITLE", "请假名称");
        colMap.put("JK_REASON", "请假事由");
...
//        }
    	...
    	List<Bean> allJkList = new ArrayList<Bean>();
        /*拼sql并查询*/
        String xmId=  paramBean.getStr("XM_ID");
        String where="and   XM_ID='"+xmId+"'  and JK_STATUS="+2;
        //	请假的数据
        List<Bean>  list=ServDao.finds("TS_XMGL_JKPASS", where);
        if(list != null && list.size() != 0){
        	for(Bean bean :list){
        		...
        		String jkName=bean.getStr("JK_NAME");
        		String jkReason=bean.getStr("JK_REASON");
        		String jkTitle=bean.getStr("JK_TITLE");
        		String jkKsName=bean.getStr("JK_KSNAME");
        		String jkCity=bean.getStr("JK_JKCITY");
        		String[] qjKsNameArry=jkKsName.split(",");
        		...
        		jkBean.set("USER_CODE", userCode);
        		jkBean.set("JK_NAME", jkName);
        		jkBean.set("JK_REASON", jkReason);
        		jkBean.set("JKKS_NAME", typeName);
        		jkBean.set("JK_TITLE", jkTitle);
        		jkBean.set("JK_JKCITY", jkCity);
        		allJkList.add(jkBean);
        	}
        }
