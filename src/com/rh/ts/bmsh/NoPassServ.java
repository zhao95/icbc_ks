	        ...
	        for (Bean rowBean : rowBeanList) {
	          ...
	          //将审核通过的数据   导入到不通过表中   
	           ...
	           if(pathflag==0){
	            ...
	            if (nopassshlist != null&&nopassshlist.size()!=0) {
	                rowBean.set(ImpUtils.ERROR_NAME, "重复数据，报名数据已在不同过表中，不需导入");
	                continue;
	            }else if (shlist != null&&shlist.size()!=0) {
	            	...
	            	if(level>shlist.get(0).getInt("SH_LEVEL")){
	    				...
	    				OutBean outbean = ServMgr.act("TS_WFS_APPLY", "backFlow",
							...
							newBean.set("SH_OTHER", allman1);
								//异议过的数据不能再进行异议
								int bmyiyi = bmbean.getInt("BM_YIYI_STATE");
								if(2==bmyiyi){
									bmbean.set("BM_SH_STATE", "3");
								}else{
