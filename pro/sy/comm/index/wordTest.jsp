...
if (ZotnClient) { 
	var ad = ZotnClient.WordVersion;
	if (ad && ad.length > 0) {//正常安装
	    jQuery("body").html("<div style='background-color:green;width:100%;height:20px;font-size:14px;line-height:15px;text-align:center;color:white;'>已经安装Word</div>");
	    parent.jQuery("#rh_test_word_iframe").css("background-color","green");
	} else {
		jQuery("body").html("<div style='margin:0px;background-color:yellow;width:100%;height:20px;font-size:14px;line-height:15px;text-align:center;color:red;'>尚未安装Word，请自行安装Microsoft Word软件</div>");
	}
} else {
