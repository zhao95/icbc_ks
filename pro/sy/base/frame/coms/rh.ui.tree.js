...
rh.ui.Tree = function(options) {
	//@TODO:考虑用系统变量替换
	this.parPath = FireFly.contextPath + "/sy/base/frame/coms/tree/";
	var opts = {
		...
		rhexpand: true, //非组件本身属性，后修改增加，用于全局参数
		rhItemCode:"",
		root_no_check:false, // 指定根不用显示checkbox
		childOnly:false, // 只能选中子节点
		rhTreeHandler: options.handler //rhDictTreeView实例传递的句柄 add by wangchen
	};
 ...
 * 取得根节点ID
 */
rh.ui.Tree.prototype.getRootNodeId = function(){
	return this.obj.find(".bbit-tree-root > li > div").attr("itemid");
}
 ...
 * 设置当前节点
 */
rh.ui.Tree.prototype.setCurrentNode = function(item) {
	this.obj.setTCT(item);
};
...
/**
 * 取消所有已被选中的checkbox，但不收起整棵树
 */
rh.ui.Tree.prototype.cleanExcluCollapse = function() {
 ...
 * 取消所有已被选中的checkbox，但不收起整棵树
 */
rh.ui.Tree.prototype.cleanAndCollapseExcluRoot = function() {
	
	var _self = this;
	
	this.obj.find("div.bbit-tree-node-expanded").each(function(){// 收起所有被展开的节点
		if(jQuery(this).attr("id") && jQuery(this).attr("id") != "" && jQuery(this).attr("id").indexOf("root") < 0) {
			jQuery(this).find("img.bbit-tree-ec-icon").each(function(){
				var path = jQuery(this).parent().attr("tpath");
				if(path != 0){
					jQuery(this).click();
				}
			});
	...
	this.obj.find(".bbit-tree-selected").each(function(){// 找到所有被选中的checkbox，取消选择
		var path = jQuery(this).attr("tpath");
		if(path) {
			_self.obj.uncheckNode(path);
		}
	});
...
rh.ui.Tree.prototype.selectNodes = function(ids) {
	var _self = this;
	setTimeout(function(){ //add by wangchen 
		// 先取消所有选中
		_self.obj.find(".bbit-tree-selected").removeClass("bbit-tree-selected");
		if(!_self._opts.showcheck) {// 单选
			// 选中指定ID的节点
//			jQuery("#" + _self.getId() + "_" + ids[0].replace(/[^\w]/gi, "_")).addClass("bbit-tree-selected");
			jQuery("#" + _self.getId() + "_" + ids[0].replace(/[^\w]/gi, "_"),_self.obj).find("[unselectable]").click();
		} else {
			jQuery.each(ids, function(index, id) {
				_self.checkNode(id);// 选中
				
				// 选中指定ID的节点
				jQuery("#" + _self.getId() + "_" + id.replace(/[^\w]/gi, "_"),_self.obj).addClass("bbit-tree-selected");
			});
			var rhDictTreeHandler = _self._opts.handler;
			for (var i=0;i<rhDictTreeHandler.errorNodes.length;i++) { // add by wangchen
				top.Tip.addTip(rhDictTreeHandler.errorNodes[i].ERR_MSG, "warn");
			}
			rhDictTreeHandler.errorNodes = [];
		}
		_self.expandParent();
	},100);
};
 ...
 * handler 作用域
 */
rh.ui.Tree.prototype.setAfterExpandFunc = function(func,handler) {
	this.obj.setAfterExpandFunc(func,handler);
};