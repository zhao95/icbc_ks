...
global.Fn.serializeJson = function(form, filter) {
	...
	fields.each(function(i) {
		...
		if (result[name]) {
			...
			result[name] = amountReg.test(this.value) ? this.value.replace(
					/\,/g, "") : this.value;
		}
	});
	return result;
...
global.Fn.formatDate = function() {
	...
	utc = true, str = arguments[0], format = arguments[1] || dFormat;
	...
	if (/(y+)/.test(format)) {
		format = format.replace(RegExp.$1, (myDate.getFullYear() + "")
				.substr(4 - RegExp.$1.length));
	}
	for ( var k in opts) {
		if (new RegExp("(" + k + ")").test(format)) {
			format = format.replace(RegExp.$1,
					(RegExp.$1.length == 1) ? (opts[k]) : (("00" + opts[k])
							.substr(("" + opts[k]).length)));
		}
	}
	return format;
}
	...
	$.validator
			.setDefaults({
				...
				errorPlacement : function(error, element) { // render error
					...
					if (element.attr("type") == "radio"
							|| element.attr("type") == "checkbox") { // for
																		...
																		// chosen
																		// container
						$beforElement = $(element).closest('.form-group')
				...
				highlight : function(element) {
					...
					// display OK icon
					if (($(element).attr("type") == "radio" || $(element).attr(
							"type") == "checkbox")
							&& $parent.children().length > 2) { // for chosen
																// elements,
																// need to
																// insert the
																// error after
						...
						$label = $parent.prev("label").removeClass(
				...
				success : function(label, element) {
					...
					if ($(element).parent().hasClass('input-group')) {
						$parent = $(element).closest('.input-group').parent();
						$label = $parent.prev("label").removeClass('has-error')
								.addClass('has-success').find('.symbol');
					} else if ($parent.children().length > 2) {
						$parent = $(element).parent();
						$label = $parent.prev("label").removeClass('has-error')
								.addClass('has-success').find('.symbol');
...
global.Fn.ShowMsg = function(options) {
	var templates = {
		dialog : "<div class='show-msg modal fade' tabindex='-1' role='dialog'>"
	...
	var defaults = {
		type : 'show:success', // Tips Type,
								// Support[show,alert,confirm,prompt,progress],
								// e.g:'alert:error',default 'show:success'
		title : '',
		msg : '',
	...
	var title = opts.title ? opts.title : style[1]
			&& style[1].replace(/\w+/, function(word) {
...
global.Fn.ShowPop = function(options) {
	...
	var defaults = {
		...
		content : "", // Support [text,html,jquery
						// object,object{type:"view|datagrid",url:"",method:"get",data:{}}]
		buttons : {
			'保存' : [ "btn-primary btnSave" ],
			'取消' : [ "btn-default btnCancel", function(pop) {
				pop.close();
			} ]
		}
	};
	...
	if (!$popups[0]) {
		...
		var popupsHTML = '<div class="arpopups'
				+ (_class ? " " + _class : "")
				+ '" id="'
				+ _selector.replace("#", "")
				+ '">\n'
		...
		if (opts.buttons.Cancel == undefined)
			opts.buttons.Cancel = defaults.buttons.Cancel;
		$
				.each(
						opts.buttons,
						function(name, value) {
	...
	if (!$overlay[0]) {
		$overlay = $('<div class="bg-overlay" data-for="' + $popups.id + '"/>')
				.appendTo("body");
		var overlaycss = {
	...
	$popups.close = function(remove) {// 是否移除html代码
		/*
		 * if (opts.content && (opts.content.type == "view" || opts.content.type ==
		 * "datagrid"))
		 */remove = true;
	...
	$popups.setPosition = function(timeout) {
		...
		var setStyle = function() {
			var shouldScrollTop = $popups.css({
				"margin-top" : $(window).scrollTop() - $popups.outerHeight()
						/ 2,
	...
	if (opts.content) {
		...
		if (opts.content) {
			if (opts.content.type == "datagrid") {
				...
				var _content = '';
				if (_showSearch) {
					var _searchKey = opts.content.queryParams
							&& opts.content.queryParams.SearchKey || "";
					_content += '<form id="popSearchForm">\n'
							+ '<div class="row form-group quick-search">\n'
							+ '<div class="input-group col-sm-6">\n'
							+ '<div class=" input-icon"><input type="text" name="sc" value="'
							+ _searchKey
							+ '" class="form-control input-sm" placeholder="请输入要搜索的值"><i class="clip-search"></i></div>\n'
				...
				$("div.modal-footer button:contains(Save)", $popups).text(
						"Select");
				$popups.list = $("#" + _listid);

				var paginationConfig = $.extend(true, {
					completeCallBack : function() {
						$popups.setPosition(1);
					}// 在datagrid渲染完成后 重新定位位置
				...
				var popDatagrid = new PagedDataTable({
					...
					// queryParameter: opts.content.pagination.queryParameter,
					// //查询的表单
					// successCallBack:
					// opts.content.pagination.successCallBack,// function
					// (data) { return data;} //查询数据成功后的回调，用于对数据的处理
					// completeCallBack: function () { $popups.setPosition(1); }
					// //在datagrid渲染完成后 重新定位位置
					// },
				...
				// 增加获取所选数据方法
				$popups.GetChecked = $.proxy(popDatagrid.GetChecked,
						popDatagrid);
				...
				$
						.ajax({
							...
							success : function(result) {
								...
								// $popups).not("[field=Check] input").iCheck({
								// checkboxClass: 'icheckbox_minimal-grey',
								// radioClass: 'iradio_minimal-grey',
								// });
								if (typeof opts.callback === "function")
									opts.callback($popups);
							},
							error : function() {
								$popups
										.setContent('<div class="pd30 center">No Info</div>');
							}
						});
			} else {
		...
		$(document).bind(
				'keydown.' + $popups.id,
				function(e) {
					var _lastpopid = $("div.arpopups:visible").last()
							.attr("id");
					var _msg = $("div.messager-window");
					if (e.keyCode === 27 && $popups.id == _lastpopid
							&& !_msg[0])
						$popups.close();
					if (e.keyCode === 13 && $popups.id == _lastpopid
							&& e.target.name != "SearchKey"
		...
		if ((opts.content && opts.content.type != "view")
...
global.Fn.SaveForm = function(options) {
	...
	if (global.Fn.$(config.form).valid()) {
		if (config.validateReadOnly) {
			var $readonlyInputs = global.Fn.$(config.form).find(
					'input[readonly]');
	...
	if (config.listenerChange) {
		...
		$(document).on(
				...
				function(e) {
					...
					global.formChange = global.formChange
							|| global.Fn.Equal(global.preFormData,
									global.curFormData);
...
global.Fn.InitPlugin1 = function(plugins, container, bmid) {
	...
	if (plugins.indexOf('img') !== -1) {
		...
		$.each($imgs, function(index, img) {
			global.Fn.InitUploadImage1($(img).parent(), bmid);
		});
	}
	if (plugins.indexOf('datetime') !== -1) {
		...
		if ($datePickers && $datePickers.length > 0) {
			$.each($datePickers, function(index, item) {
				...
				var startView = parseInt($cur.data('startview') || minViewModel
						|| 0);
				$cur.datepicker({
					language : 'zh-CN',
					...
					// defaultViewDate:1,//0,year: the current year、month: 1
					// 、day: 1
					minViewMode : minViewModel,// “days” or 0, “months” or 1,
												// and “years” or 2
					startView : startView,// 0 day,1 month ,2 year
				}).next("span.input-group-btn").bind("click", function() {
	...
	if (plugins.indexOf('ckeditor') !== -1) {
		...
		$.each(ckeditors, function(index, ck) {
			CKEDITOR.replace(ck.name, {
				filebrowserUploadUrl : global.Fn.$(ck.id).data('url')
						|| (http_server + 'admin/common/ckeditorimgupload.php')
			});
		});
	}
}
global.Fn.InitUploadImage1 = function(container, bmid) {
						...
						if (arr[1] != "xls" && arr[1] != "xlsx") {
							...
							$(".upload-list>ul", $upload)
									.append(
											'<div width:"100px" id='
													+ s
													+ '><li>&nbsp;&nbsp;<a style="font-size:13px" href="/file/'
													+ s
...
global.Fn.isJsonOrArray = function(obj) {
	return typeof (obj) == "object"
			&& Object.prototype.toString.call(obj).toLowerCase() == "[object object]"
			&& !obj.length || toString.apply(obj) == '[object Array]';
}
