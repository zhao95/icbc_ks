...
//---------处理岗位序列字段-------//
//获取初始岗位序列
var selectedPostionSequence = _viewer.getItem("STATION_NO").getValue();

//根据岗位类加载序列选项 并选中初始岗位序列
function loadPostionSequenceOption() {
    var postionType = _viewer.getItem("STATION_TYPE").getValue();
    ...
    if (postionType) {
        ...
        _viewer.getItem("STATION_NO").addOptions(sequenceList);
        //如果有初始选中的岗位序列选中
        if (selectedPostionSequence) {
            _viewer.getItem("STATION_NO").select(selectedPostionSequence);
        }
    }
}
