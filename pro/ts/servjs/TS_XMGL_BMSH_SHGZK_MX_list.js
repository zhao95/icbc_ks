		...
		if(value1 == 1 && val.length >0) {
			...
			if(obj2[0].type=="select"||obj2[0].type=="date"){
				$(item).find("td[icode='BUTTONS']").append(
						...
						'<a class="rhGrid-td-rowBtnObj rh-icon" id="TS_XMGL_BMSH_SHGZK_MX-copy" actcode="copy" rowpk="'+dataId+'" rowjson="'+val+'" rowname="'+name+'" >'+
...
function bindCard() {
	//当行编辑事件
	jQuery("td [id='TS_XMGL_BMSH_SHGZK_MX-setting']").unbind("click").bind("click", function() {
		...
		var nameArg = name.split("#"+obj2[0].vari+"#");
			...
			var result = FireFly.byId("TS_XMGL_BMSH_SHGZK_MX",idinfo);
...
//					debugger;
					...
					if(""!=mx_name1){
						...
						FireFly.doAct("TS_XMGL_BMSH_SHGZK_MX","saveinfo",param);
					}else{
						alert("请输入提示信息");
						return false;
					}
	...
	jQuery("td [id='TS_XMGL_BMSH_SHGZK_MX-copy']").unbind("click").bind("click", function() {
		var dataId = jQuery(this).attr("rowpk");
		var param = {};
		param["dataId"]=dataId;
		FireFly.doAct("TS_XMGL_BMSH_SHGZK_MX","CopyMx",param);
		_viewer.refresh();
	});
...
function bindelete(){
	...
	$("#chongzhi").click(function(){
		...
		var param = {};
		param["GZ_ID"]="N03";
		var result = FireFly.doAct("TS_XMGL_BMSH_SHGZK_MX","getJkgz",param);
		var mx_name = result.gzbean;
		$("#RULE-VAR-INPUT").val("");
		$("#RULE-VAR-INPUT").val(mx_name);
	});
