...
$("#TS_XMGL_JKPASS-expAll").unbind('click').bind('click', function () {
    ...
    var data = {XM_ID: xmId};
    window.open(FireFly.getContextPath() + '/TS_XMGL_JKPASS.expAll.do?data=' +
...
//从excel中导入人员
const IMPORT_FILE_ID = "TS_XMGL_JKPASS-impUserByExcel";
...
$impFile.unbind('click').bind('click', function () {
    window.open(FireFly.getContextPath() + '/ts/imp_template/项目借考人员导入模板.xls');
});


$('#' + IMPORT_FILE_ID).remove();
_viewer.getBtn('impFile').remove();