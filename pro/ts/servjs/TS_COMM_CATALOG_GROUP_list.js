...
//每一行添加编辑和删除
$("#TS_COMM_CATALOG_GROUP .rhGrid").find("tr").each(function(index, item) {
	if(index != 0){
		var dataId = item.id;
		
		$(item).find("td[icode='BUTTONS']").append(
				'<a class="rhGrid-td-rowBtnObj rh-icon" id="TS_COMM_CATALOG_GROUP-upd" actcode="upd" rowpk="'+dataId+'"><span class="rh-icon-inner">编辑</span><span class="rh-icon-img btn-edit"></span></a>'
				);
		// 为每个按钮绑定卡片
		bindCard();
	}
...
function bindCard() {	
	//当行编辑事件
	jQuery("td [id='TS_COMM_CATALOG_GROUP-upd']").unbind("click").bind("click", function() {
