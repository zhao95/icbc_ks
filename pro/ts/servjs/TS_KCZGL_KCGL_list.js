...
_viewer.getBtn("add").unbind("click").bind("click",function(event){
	...
	var configStr = "TS_KCGL,{'TARGET':'','SOURCE':'KC_ID~KC_CODE~KC_NAME~KC_ODEPTNAME'," +
			"'HIDE':'KC_ID','TYPE':'multi','HTMLITEM':'','EXTWHERE':'and KC_STATE = 5 and (kc_type = 1 or kc_type is null)'}";
	var options = {
		"config" :configStr,
		"parHandler":_viewer,
...
//		'<a class="rhGrid-td-rowBtnObj " operCode="optDeleteBtn" rowpk="'+dataId+'" style="cursor:pointer">删除&nbsp</a>';
		...
		var hoverDiv = "<div class='hoverDiv' id='hoverDiv_"+dataId+"' style='height: "+divHeight+"px; line-height: "+(divHeight-14)+"px; display: none;color:#222222;'>"+abtns+"</div>";
...
_viewer.getBtn("imp").unbind("click").bind("click",function() {
	var groupId = _viewer.getParHandler().getItem("GROUP_ID").getValue();
