_viewer=this;
$("#TS_BMSH_RULE_KXLGZ_GZK-POSTION_NAME").css("cursor","pointer");
$("#TS_BMSH_RULE_KXLGZ_GZK-POSTION_NAME").unbind('click').bind('click',function(){
    var xls = $("#TS_BMSH_RULE_KXLGZ_GZK-POSTION_SEQUENCE__NAME").val();
    var lb = $("#TS_BMSH_RULE_KXLGZ_GZK-POSTION_TYPE__NAME").val();
    var extwhere = "";
    if(lb==""){
    	alert("请先选择类别");
    	return false;
	...
	var options = {
		    ...
		    "replaceCallBack":function(idArray) {//回调，idArray为选中记录的相应字段的数组集合
		    	...
		    	$("#TS_BMSH_RULE_KXLGZ_GZK-POSTION_NAME").val(ids);
			}
		};
